...
function unify(t1: Type, t2: Type): Substitution {
  ...
  if (t1.t === "tTup" && t2.t === "tTup" && t1.ts.length === t2.ts.length) {
    return t1.ts.reduce(
      (s, t, i) =>
        composeSubst(s, unify(applySubst(s, t), applySubst(s, t2.ts[i]))),
      emptySubst
    );
  }
  throw (
    "type mismatch between " + AST.prettyType(t1) + " and " + AST.prettyType(t2)
  );
}
...
function match(ctx: Context, pat: AST.Pattern, sch: Scheme): Context {
  console.log(
    `matching ${AST.prettyPattern(pat)} with ${AST.prettyScheme(sch)}`
  );
  switch (pat.t) {
    case "pTup":
      switch (sch.t.t) {
